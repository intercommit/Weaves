...
function createNotice(growler, msg, options){
	...
	var imageDiv = new Element("div",{"style":"float:left; padding: 5px;"});
	if (opt.image) {
		var image = new Element("img",{ "src": opt.image});
		imageDiv.insert(image);
	}
	container.insert(imageDiv);
	var textDiv = new Element("div");
	
	textDiv.insert(new Element("div", {"class": "Growler-notice-head"}).update(opt.header));
	textDiv.insert(new Element("div", {"class": "Growler-notice-body"}).update(msg));
	container.insert(textDiv);
